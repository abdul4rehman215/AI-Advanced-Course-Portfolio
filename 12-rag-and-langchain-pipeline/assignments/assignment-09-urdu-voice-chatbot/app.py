...
def format_sources(source_docs) -> List[str]:
    ...
    for doc in source_docs:
        source_name = doc.metadata.get("source_name") or Path(
...
with st.sidebar:
    ...
    default_api_key = get_google_api_key()
    ...
    if build_clicked:
        if not google_api_key:
            ...
            with st.spinner(
                "Loading PDFs, splitting content, creating embeddings, and building the vector store..."
            ):
                try:
